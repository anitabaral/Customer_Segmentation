...
def rfm_data():
    ...
    Returns:
        tuple: Country and segment selected by the user.
    """
...
class RFMVisualizations:
    ...
    def show_aggregated_data(self, individual_segment: pd.DataFrame):
        ...
        if individual_segment is None:
            ...
            st.markdown(f"**1. Total number of customers: {int(mean_count['recency'].loc['count'])}**")
    ...
    def show_customer_dataframe(self, preprocessed_data: pd.DataFrame, segment: pd.DataFrame):
        ...
        segments = preprocessed_data.loc[preprocessed_data.customer_id.isin(segment.customer_id)]
        grouped_segments = segments.groupby(['customer_id']).first().reset_index().sort_values('spend_amount', ascending=False)
        st.markdown("##")
        st.subheader('Nations individual segments')
        st.markdown('All the customers belonging to the segment of the given nation.')      
        with st.sidebar:
            submit = st.selectbox('Select the top candidates option', ('All candidates', 'Top Candidates')) 
